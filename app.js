...
const initializeDbAndServer = async () => {
  try {
    ...
    app.listen(3000, () => {
      console.log('Server is running at https://keep.google.com/u/0/')
    })
  } catch (error) {
...
//API 1 RETURN LIST OF TODOS
app.get('/todos/', async (request, response) => {
  const {search_q = '', priority, status, category} = request.query

  let data = null
  let getTodosQuery = ''
  // WE ARE USING SWITCH CASES FOR BECAUSE OF DIFFERENT SCENARIO

  switch (true) {
    // SCENARIO 1 HAS ONLY USE STATUS
...
app.put('/todos/:todoId/', async (request, response) => {
  ...
  let updateTodo
  //WE ARE USING SWITCH CASE BECAUSE SCENARIO ARE GIVEN
  switch (true) {
    // UPDATING STATUS
